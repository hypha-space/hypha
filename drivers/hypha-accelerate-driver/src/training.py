...
def main(socket_path: str, work_dir: str, job_json: str) -> None:
    ...
    with Session(socket_path) as session:
        ...
        response = session.fetch(job_spec["model"])
        print(response)
        ...
        print(response)
        print(os.listdir(f"{work_dir}/artifacts"))
        ds = load_file(f"{work_dir}/artifacts/{job_spec['data']['filenames'][0]}")
        data_loader = torch.utils.data.DataLoader(
            torch.utils.data.TensorDataset(ds["inputs"][:6]), batch_size=job_spec["config"]["batch_size"]
        )

        model, optimizer, training_dataloader, scheduler = accelerator.prepare(model, optimizer, data_loader, scheduler)
        run_epoch = get_training_loop(job_spec["config"])
