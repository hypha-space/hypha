...
Gateways participate in all network protocols (Kademlia, Gossipsub, request/response) but do not execute compute tasks or store data. They require minimal resources but benefit from public IP addresses and reliable connectivity. See the [libp2p Network Stack RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-03-25-libp2p_network_stack.md) for details on the underlying P2P networking foundation.

## Scheduler

Schedulers orchestrate distributed training jobs. Their responsibilities include:

**Resource Discovery & Allocation**: Schedulers discover and allocate workers through a decentralized pub/sub mechanism (see the [Decentralized Resource Allocation Protocol RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-08-04_decentralized_resource_allocation_protocol.md)). They publish resource requirements via Gossipsub on the `hypha/worker` topic, specifying needs like GPU, CPU, memory, executor types, and pricing. Workers respond with offers if they can satisfy the requirements. The scheduler collects these offers and selects the best matches using greedy selection (lowest price first) for straightforward resource allocation.
...
**Training Coordination**: Tracking worker progress and determining optimal synchronization points. The scheduler simulates future completion times based on reported batch processing speeds to minimize stragglers and respond to varying worker hardware characteristics. See the [Performance-Aware Scheduling RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-10-16_performance_aware_scheduling.md) for details on handling heterogeneous hardware.
...
**Job Bridge**: Provides a language-agnostic HTTP API over Unix sockets, allowing executors to fetch resources, send data to peers, and receive streams without understanding P2P protocols. See the [Job Bridge RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-08-14_job_bridge.md) for the complete interface specification.

Workers can run multiple executors simultaneously, enabling diverse workload support on a single machine.

## Data

Data nodes serve slices from prepared datasets to workers on demand. See the [Data Nodes RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-10-07_data_nodes.md) for the complete specification.
...
* [Decentralized Resource Allocation Protocol RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-08-04_decentralized_resource_allocation_protocol.md) — Pub/sub-based protocol for decentralized scheduler-worker matching, including message flow, lease management, and pricing mechanisms.
* [Job Bridge RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-08-14_job_bridge.md) — Language-agnostic HTTP API enabling executors to fetch resources and communicate with the network without understanding P2P protocols.
* [Data Nodes RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-10-07_data_nodes.md) — Architecture for serving prepared datasets via DHT announcement and SafeTensors streaming.
* [Performance-Aware Scheduling RFC](https://github.com/hypha-space/hypha/blob/alpha/rfc/2025-10-16_performance_aware_scheduling.md) — Two-phase scheduling system for heterogeneous hardware with adaptive DiLoCo synchronization points.
