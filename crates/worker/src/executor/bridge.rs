...
async fn send_resource(
    ...
    while let Some(item) = writers.next().await.transpose().map_err(Error::Connector)? {
        ...
        let sent_bytes = futures_util::io::copy(&mut reader, &mut writer).await?;
        tracing::info!(size = sent_bytes, file = %abs.display(), "Sent resource");
        futures_util::io::AsyncWriteExt::flush(&mut writer).await?;
        futures_util::io::AsyncWriteExt::close(&mut writer).await?;
