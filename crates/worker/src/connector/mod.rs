...
where
    T: Clone + StreamPullInterface + StreamPullSenderInterface + Send + Sync + 'static,
{
    network: T,
}
        ...
        Box::pin(async move {
            match fetch.as_ref() {
                Reference::Peers {
                    ...
                    let stream = self
                        .network
                        .stream_pull(peer, resource.as_ref().expect("resource").as_str())
                        .await?;
                    let item = ReadItem {
                        meta: ItemMeta {
                            kind: "peer",
