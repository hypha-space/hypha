...
mod tests {
    ...
    #[tokio::test]
    async fn test_worker_network_creation_with_relay_and_dcutr() {
        // This test verifies that the worker Network struct can be created with relay and DCUtR components
    ...
    #[tokio::test]
    async fn test_worker_behaviour_contains_relay_and_dcutr() {
    ...
    #[tokio::test]
    async fn test_worker_relay_client_configuration() {
        // Test that verifies the worker relay client can be properly configured
        let keypair = create_test_keypair();
        let peer_id = keypair.public().to_peer_id();
