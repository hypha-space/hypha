...
pub trait StreamPullReceiverInterface: StreamPullInterface {
    ...
    fn streams_pull(
        &self,
    ) -> Result<impl Stream<Item = (PeerId, String, impl AsyncWrite)>, AlreadyRegistered> {
        let incoming_streams = self.stream_control().accept(TENSOR_STREAM_PROTOCOL)?.then(
            |(peer_id, mut stream)| async move {
                let mut resource_len = [0u8; 8];
                stream
                    .read_exact(&mut resource_len)
                    .await
                    .expect("resource length can be read");
                let resource_len = u64::from_le_bytes(resource_len);

                let mut resource = vec![0; resource_len as usize];

                stream
                    .read_exact(&mut resource)
                    .await
                    .expect("stream can be read");

                let resource = String::from_utf8(resource).expect("resource is a string");
                (peer_id, resource, stream.compat())
            },
        );
...
pub trait StreamPullSenderInterface: StreamPullInterface + Sync {
    ...
    fn stream_pull(
        ...
        async move {
            ...
            let resource_bytes = resource_name.as_bytes();
