...
async fn run(config: ConfigWithMetadata<Config>) -> Result<()> {
    ...
    // If the dataset path is a file, treat the file as a dataset,
    // if it is a directory, treat each file in the directory as a subset of the dataset.
    ...
    let (dataset_name, dataset_files) = if dataset_path.is_dir() {
        (
            dataset_path
                .file_name()
                .expect("a file name")
        ...
        |(peer_id, resource, mut stream)| {
            ...
            async move {
                ...
                if dataset_name == resource.dataset.as_str() {
                    ...
                    let dataset_slice = &dataset_files[resource.offsets.0 as usize..resource.offsets.1 as usize];

                    match dataset_slice.iter().map(|file| {
                        tensor::TensorData::try_open(file.as_path())
                    }).collect::<Result<Vec<_>, _>>() {
                        Ok(dataset_slice_data) => {
                            match dataset_slice_data.iter().map(|data| data.tensors()).collect::<Result<Vec<_>, _>>() {
                                Ok(tensors) => {
                                    let tensors = tensors.into_iter().flatten();

                                    // Send all selected dataset files as a single safetensor.
                                    if let Err(e) = tensor::serialize(tensors, &None, &mut stream).await {
                                        tracing::warn!(peer_id = %peer_id, dataset = resource.dataset, "Failed to serialize dataset: {}", e);
                                    }
                                }
                                Err(e) => {
                                    tracing::warn!(peer_id = %peer_id, dataset = resource.dataset, "Failed to open tensor data: {}", e);
                                }
                            }
                        }
                        Err(e) => {
                            tracing::warn!(peer_id = %peer_id, dataset = resource.dataset, "Failed to open dataset: {}", e);
                        }
                    }
                } else {
